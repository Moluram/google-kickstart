...
public class Solution {
    ...
    private static List<Integer> solve(int N, int X, long[] A) {
        var Ad = new ArrayList<Pair<Integer, Long>>();

        for (int i = 1; i <= N; i++) {
            Ad.add(pair(i, (long) Math.ceil(A[i - 1] / (double) X)));
        }

        return Ad.stream()
                .sorted((p1, p2) -> {
                    final int compare = Long.compare(p1.second, p2.second);
