package rounds.Y2020.F.MetalHarvest;
...
public class Solution {
    ...
    private static Long solve(int N, long K, List<Pair<Long, Long>> SE) {
        long result = 0;

        SE.sort(Comparator.comparingLong(Pair::getFirst));

        long start = SE.get(0).getFirst();
        long end = SE.get(0).getSecond();
        ...
        while (finished < N) {
            ...
            if (K < interval) {
                long n = (long) Math.ceil(interval / (double) K);
                result += n;
            ...
            while (finished < N && SE.get(finished).getSecond() <= start) {
                finished++;
            }
            ...
            end = SE.get(finished).getSecond();

            if (start < SE.get(finished).getFirst()) {
                start = SE.get(finished).getFirst();
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (int caseId = 1; caseId <= numCases; caseId++) {
            ...
            List<Pair<Long, Long>> SE = new ArrayList<>();

            for (int i = 0; i < N; i++) {
                SE.add(pair(sc.nextLong(), sc.nextLong()));
            }
            ////

            var ans = solve(N, K, SE);
