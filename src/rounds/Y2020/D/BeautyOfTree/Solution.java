package rounds.Y2020.D.BeautyOfTree;
...
public class Solution {
    ...
    private static double solve(int A, int B, ArrayList<Integer> nodes) {
        var pairs = new ArrayList<Pair<Integer, Integer>>();

        final int size = nodes.size() + 1;

        for (int i = 2; i <= size; i++) {
            pairs.add(pair(i, nodes.get(i - 2)));
        }
        pairs.add(pair(1, 0));

        pairs.sort((p1, p2) -> Integer.compare(p1.second, p2.second) * -1);

        final double[] Av = new double[size + 1];
        final double[] Bv = new double[size + 1];
        sumChances(A, B, nodes, pairs, size, Av, Bv);

        double result = 0;

        for (int i = 0; i <= size; i++) {
            result += 1 - ((1 - Av[i] / size) * (1 - Bv[i] / size));
        }

        return BigDecimal.valueOf(result).round(new MathContext(7)).doubleValue();
    }

    private static void sumChances(int a, int b, ArrayList<Integer> nodes,
            ArrayList<Pair<Integer, Integer>> pairs, int size,
            double[] av, double[] bv) {
        final HashMap<Pair<Integer, Integer>, Integer> memo = new HashMap<>();
        for (var pair : pairs) {
            bv[pair.first]++;
            av[pair.first]++;
            ...
            int counter = a - 1;
            while (position > 0 && counter > 0) {
                if (memo.containsKey(pair(position, counter)))
                position = position - 2;
                counter--;
