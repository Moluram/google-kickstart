package rounds.Y2020.G.Kick_Start;
...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for (int caseId = 1; caseId <= numCases; caseId++) {
            ...
            var ans = solve(line, 0, line.length());
    ...
    private static long solve(String line, int start, int end) {
        int[] kick = new int[line.length()];

        if (line.length() < 9) return 0;

        if (line.charAt(0) == 'K' && line.charAt(1) == 'I'
                && line.charAt(2) == 'C' && line.charAt(3) == 'K') {
            kick[0]++;
        }

        for (int i = 1; i < line.length(); i++) {
            if (i < line.length() - 4 && line.charAt(i) == 'K' && line.charAt(i + 1) == 'I'
                && line.charAt(i + 2) == 'C' && line.charAt(i + 3) == 'K') {
                kick[i]++;
            }
            kick[i] += kick[i - 1];
        }
        ...
        for (int i = line.length(); i > 4; i--) {
            if (line.charAt(i - 4) == 'S' && line.charAt(i - 3) == 'T'
                    && line.charAt(i - 2) == 'A' && line.charAt(i - 1) == 'R'
                    && line.charAt(i) == 'T'){
                ans += kick[i - 1];
            }
        }
