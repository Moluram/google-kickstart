package rounds.Y2021.C.SmallerStrings;
...
public class Solution {
    ...
    private static final Map<Integer, Map<Integer, BigInteger>> memoPow = new HashMap<>();

    private static final BigInteger edge = BigInteger
            .valueOf(10).pow(9).add(BigInteger.valueOf(7));

    private static BigInteger solve(int N, int K, String s) {
        if (N == 1) {
            return BigInteger.valueOf(Math.min(s.charAt(0) - 'a', K));
        }
        ...
        final int mid = N % 2 == 0 ? N / 2 - 1 : N / 2;

        final char[] chars = s.toCharArray();

        for (int i = mid; i >= 0; i--) {
            result = result.add(BigInteger.valueOf(chars[i] - 'a').multiply(kPow));
            kPow = kPow.multiply(k);
        }

        String s1 = s.substring(0, mid + 1);

        if (N % 2 != 0) {
            s1 = s1 + new StringBuilder(s1.substring(0, s1.length() - 1)).reverse();
        } else  {
            s1 = s1 + new StringBuilder(s1).reverse();
        }

        if (s1.compareTo(s) < 0) result = result.add(ONE);

        if (result.compareTo(edge) >= 0) result = result.mod(edge);
